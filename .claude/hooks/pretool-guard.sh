#!/usr/bin/env bash
# pretool-guard.sh — PreToolUse(Bash) 기계 가드 (workspace-builder R23, axwith-business 실측 패턴)
#  (1) 비가역 명령 차단. 명령 문자열의 마커를 사용자 승인으로 해석하지 않는다.
#   추가 패턴 (생성 인터뷰에서 식별한 비가역 외부 자원): npm publish, wrangler deploy, wrangler secret put, git push
#  (2) git add/commit 시 PII 의심 파일(확장자·키워드) 검사 → 차단(우회 없음 — 이력 비가역).
# bypassPermissions 모드에서도 PreToolUse 훅은 실행됨 — 이 파일이 유일한 기계 방어선.
# 등록: settings.guard.json.template 참조 (settings.local.json에 병합).
set -uo pipefail
...
# (1) 비가역 명령 가드. 환경변수·주석 마커로 우회할 수 없다.
if printf '%s' "$CMD" | grep -qE '(rm[[:space:]]+-r[fF])|(git[[:space:]]+push[[:space:]]+.*--force)|(git[[:space:]]+reset[[:space:]]+--hard)|(find[[:space:]]+.*-delete)|(npm publish)|(wrangler deploy)|(wrangler secret put)|(git push)'; then
  echo "WB_POLICY_DENY" >&2
  echo "[pretool-guard] 차단된 작업은 영향 범위를 검토하고 승인된 별도 실행 경로에서 수행하세요. 명령 문자열은 승인 증거가 아닙니다." >&2
  exit 2
fi

# (2) git add/commit PII 가드 — staged + 미추적 경로에서 PII 패턴 검사 (우회 마커 없음)
if printf '%s' "$CMD" | grep -qE '(^|[;&|[:space:]])git[[:space:]]+(commit|add)'; then
  DIR="${CLAUDE_PROJECT_DIR:-$(pwd)}"
  PII=$( (cd "$DIR" && { git -c core.quotePath=false diff --cached --name-only; git -c core.quotePath=false status --porcelain | sed 's/^...//'; } 2>/dev/null) \
        | grep -iE '\.(pem|key|p12)$|secret|credential|password|통장|계좌|주민|신분증|이력서|계약서|동의서|개인정보' \
        | sort -u | head -10 || true)
  if [ -n "$PII" ]; then
    {
      echo "WB_POLICY_DENY"
      echo "⛔ [pretool-guard] PII/시크릿 의심 파일이 커밋 경로에 있음 — git 이력에 박히면 filter-repo 없이 제거 불가:"
      printf '%s\n' "$PII"
      echo "  .gitignore 3중 방어(확장자·키워드·폴더) 보강 또는 해당 파일 이동 후 재시도."
    } >&2
    exit 2
  fi
fi
