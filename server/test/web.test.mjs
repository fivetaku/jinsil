...
test('메인: 슬로건 띠·요금제 카드·스티커 5종·가성비 막대·순위·참여 로그·이론적 상한 표기', async () => {
  ...
  for (const s of ['클로드에게 진실을 요구합니다', '구독료', '가성비 1위', '구독료의', '비교 기준', '가격 2배 → 가치', '광고보다 적음', '측정 대기 · 통계 반영 1/2',
    ...
    '참여자가 로컬에서 관측', '0.2 측정 방식 변경', '보조 지표 · 토큰 종류별 게이지 가중치', '측정 대기 — 첫 계산', 'npx jinsil@latest setup', '증거 묶음(JSON)', '측정 범위', '정밀', '통계에서 뺀 창', '외부 사용 의심', '+5%p 잠정'])
...
test('방법론: 계산식·스티커 규칙·한계·개인정보', async () => {
  ...
  for (const s of ['30일 환산', '가성비 배수', '한도 창', 'C⁻', '잠정', '정밀', '외부 사용 의심', '가중치', '상대 가중치', '반영 지연', '--proxy', '이론적 상한', '한계', '개인정보', 'api.anthropic.com']) assert.ok(t.includes(s), s);
