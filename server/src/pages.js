...
<h2>계정 풀 사용자</h2><p>대화 파일에는 어느 계정으로 나갔는지가 남지 않습니다. 요청마다 계정을 바꾸는 풀(teamclaude)을 쓰면 <code>jinsil setup --teamclaude</code>로 풀이 남기는 요청별 로그(계정·모델·토큰)를 읽고, 풀에 등록된 계정마다 게이지를 5분마다 조회합니다. 요청 경로에는 끼지 않습니다. 로그에 캐시 보관 시간(5분/1시간)이 없어 캐시 쓰기는 5분 단가로 가정하며, 이 창은 수집기 이름(teamclaude)으로 증거 묶음에 표시됩니다.</p>
