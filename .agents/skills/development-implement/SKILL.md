---
name: development-implement
description: "코드 구현, 부분 수정, 버그 수정, LLM 통합 보완, 이어서 구현 요청에 사용합니다. '구현해줘', '버그 고쳐줘', 'RAG 통합 보완해줘' 같은 요청에 대응합니다. 단, 검증 실행은 $development-test를 사용합니다."
---

# implement Workflow

Codex-compatible mirror of `implement.md` from the `development` archetype.

Invoke explicitly with `$development-implement` followed by the workflow arguments.
...
- 구현 범위가 spec 밖이면 먼저 `$development-specify` 또는 `$development-design`으로 넘깁니다.
...
- 후속 커맨드로 `$development-test`를 안내합니다.
...
- "기능 명세부터 잡아줘" → `$development-specify`
- "설계 다시 해줘" → `$development-design`
