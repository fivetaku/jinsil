...
description: "테스트 작성, eval harness 실행, 실패 재현, 커버리지 보완, 다시 실행 요청에 사용합니다. '테스트 추가해줘', 'eval 돌려줘', '실패 재현해줘' 같은 요청에 대응합니다. 단, 보안 리뷰는 $development-review를 사용합니다."
---

# test Workflow

Codex-compatible mirror of `test.md` from the `development` archetype.

Invoke explicitly with `$development-test` followed by the workflow arguments.
...
```text
$development-test [기능명]
$development-test [기능명] --type [unit|integration|e2e|eval]
```
...
- 구현이 없으면 `$development-implement`를 먼저 실행합니다.
...
- 후속 수정은 `$development-implement`, 기준 변경은 `$development-specify`로 넘깁니다.
...
- "명세 수정해줘" → `$development-specify`
- "설계 보완해줘" → `$development-design`
- "버그 고쳐줘" → `$development-implement`
- "OWASP 리뷰해줘" → `$development-review`
