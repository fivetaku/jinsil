...
description: "코드 리뷰, 보안 점검, OWASP 검토, SOLID 위반 확인, RAG 품질 점검, 다시 리뷰 요청에 사용합니다. '리뷰해줘', 'OWASP 기준으로 봐줘', 'RAG 품질 확인해줘' 같은 요청에 대응합니다. 단, 수정 구현은 $development-implement를 사용합니다."
---

# review Workflow

Codex-compatible mirror of `review.md` from the `development` archetype.

Invoke explicitly with `$development-review` followed by the workflow arguments.
...
```text
$development-review [기능명]
$development-review --all
```
...
- 수정은 `$development-implement`, 테스트 보강은 `$development-test`, 명세 변경은 `$development-specify`로 넘깁니다.
...
- "명세 다시 써줘" → `$development-specify`
- "설계 다시 잡아줘" → `$development-design`
- "코드 고쳐줘" → `$development-implement`
